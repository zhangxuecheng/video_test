...
class SVDUNet(torch.nn.Module):
    ...
    def forward(self, sample, timestep, encoder_hidden_states, add_time_id, use_gradient_checkpointing=False, **kwargs):
        ...
        t_emb = self.time_proj(timestep).to(sample.dtype) # 位置编码，time转向量，U-Net 会先用正弦编码（Transformer 里的位置编码）time_proj 来将时刻转为向量
        t_emb = self.time_embedding(t_emb) #线性层

        # 额外约束（帧率、运动程度、噪声增强强）
        add_embeds = self.add_time_proj(add_time_id.flatten()).to(sample.dtype) #位置编码，约束转向量
        add_embeds = add_embeds.reshape((-1, 768))
        add_embeds = self.add_time_embedding(add_embeds)

        time_emb = t_emb + add_embeds # 时间约束+额外约束

        # 2. pre-process
        height, width = sample.shape[2], sample.shape[3]
        hidden_states = self.conv_in(sample)
        text_emb = encoder_hidden_states # VisonCLIP编码后的文本语义
