...
#Finetuning the whole model requires 80G VRAM. If you just finetune the temporal layers, and reduce the resolution, 32G is also enough.
# https://zhuanlan.zhihu.com/p/701223363

class TextVideoDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, metadata_path, steps_per_epoch=10000, training_shapes=[(128, 1, 128, 512, 512)]):#NCHW
    ...
    def __getitem__(self, index):
        ...
        for training_shape_id in range(len(self.training_shapes)):
            while True:
                ...
                try:
                    data = self.load_video(video_file, training_shape_id) # CTHW
                except:
                    data = None
                if data is not None:
                    break
...
class LightningModel(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, svd_ckpt_path=None, add_positional_conv=128, contrast_enhance_scale=1.01):
        ...
        self.image_encoder: SVDImageEncoder = model_manager.image_encoder #svd 图像编码器
        self.image_encoder.eval()
        self.image_encoder.requires_grad_(False)

        self.unet: SVDUNet = model_manager.unet
        self.unet.train()
        self.unet.requires_grad_(False) #冻结
        for block in self.unet.blocks:
            if isinstance(block, TemporalAttentionBlock): #训练时序注意力
        ...
        self.noise_scheduler = ContinuousODEScheduler(num_inference_steps=1000) #ODE采样，传统scheduler控制学习率, 扩散模型主要采样噪声
    ...
    def encode_video_with_vae(self, video):
        video = video.to(device=self.device, dtype=self.dtype) #CTHW
        video = video.unsqueeze(0) #BCTHW
        latents = self.vae_encoder.encode_video(video) #
        # https://blog.csdn.net/qq_42859149/article/details/124643378
        latents = rearrange(latents[0], "C T H W -> T C H W")  # latents[0]: CTHW, T:number of frames
        return latents
    ...
    def calculate_loss(self, frames): #frames:CTHW
        with torch.no_grad():
            # Call video encoder
            latents = self.encode_video_with_vae(frames) #视频2D VAE编码, TCHW
            # 图片VAE编码(repeat latents[0] TCHW, T=latents.T)
            # VAE编码前会乘scaling_factor系数，这里要除掉
            image_latents = repeat(latents[0] / self.vae_encoder.scaling_factor, "C H W -> T C H W", T=frames.shape[1])

            # CLIP 图像编码器得到约束图像的嵌入，(CHW=>TCHW, T=1)
            image_emb_clip_hidde_state = self.encode_image_with_clip(frames[:,0].unsqueeze(0)) # 交叉注意力层约束,注入SVD UNet(交叉注意力),encoder_hidden_states

            # Call scheduler
            timestep = torch.randint(0, len(self.noise_scheduler.timesteps), (1,))[0]
            timestep = self.noise_scheduler.timesteps[timestep]

            noise = torch.randn_like(latents) # 随机噪声
            noisy_latents = self.noise_scheduler.add_noise(latents, noise, timestep) # lantents加噪

            # Prepare positional id
            fps = 30
            motion_bucket_id = self.motion_bucket_manager(frames.unsqueeze(0))[0]
            noise_aug_strength = 0
            #约束条件拼接（帧率、运动程度、噪声增强强），fps-1：因为训练的时候模型实际上输入的额外约束是 fps - 1
            add_time_id = torch.tensor([[fps-1, motion_bucket_id, noise_aug_strength]], device=self.device)

        # Calculate loss
        # 将加噪latent和条件latents（每帧条件一致都是第一帧图像）拼接
        latents_input = torch.cat([noisy_latents, image_latents], dim=1) # channel concat??


        model_pred = self.unet(latents_input, timestep, image_emb_clip_hidde_state, add_time_id, use_gradient_checkpointing=True) # predict noise
        latents_output = self.noise_scheduler.step(model_pred.float(), timestep, noisy_latents.float(), to_final=True) # denoised latent
        # latents-gt and  predict losss?
        loss = torch.nn.functional.mse_loss(latents_output, latents.float() * self.contrast_enhance_scale, reduction="mean")

        # Re-weighting
        reweighted_loss = loss * self.noise_scheduler.training_weight(timestep) # 根据时间对loss加权
        return loss, reweighted_loss

    '''
     训练一批数据并反向传播。参数如下：
     batch (Tensor | (Tensor, …) | [Tensor, …]) – 数据输入，一般为（x, y） = batch。
     batch_idx (int) – 批次索引。
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(trainable_modules, lr=self.learning_rate) # 将待训练的block加入优化器
        return optimizer
