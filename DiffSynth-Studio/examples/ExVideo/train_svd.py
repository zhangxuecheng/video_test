...
def main():
    ...
    noise_scheduler = EulerDiscreteScheduler.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="scheduler")  # 噪声调度器
    ...
    vae = AutoencoderKLTemporalDecoder.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision, variant="fp16") #vae encoder
    ...
    for name, para in unet.named_parameters():
        if 'temporal_transformer_block' in name:
            ...
            para.requires_grad = True #只微调时序transfomer block，其他冻结
        else:
    ...
    train_dataset = DummyDataset(width=args.width, height=args.height, sample_frames=args.num_frames) # 25 帧
    ...
    ) #diffusers lr scheduler

    # Prepare everything with our `accelerator`. (加速对象)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                conditional_pixel_values = pixel_values[:, 0:1, :, :, :] # TCHW?

                latents = tensor_to_vae_latent(pixel_values, vae) # VAE编码
                ...
                # Backpropagate
                accelerator.backward(loss) #loss bp
